...
import { lobstersTagUrl } from '@/lib/lobsters'
import { attachAuthorAvatars, fetchTagStories, type LobstersStory } from '@/lib/story-feed'
...
export default async function TagStoriesPage({ params, searchParams }: PageProps) {
  const [{ id }, query] = await Promise.all([params, searchParams])
  const tag = normalizeQuery(id)
  ...
  if (!tag) {
    feedError = 'A tag id is required.'
  } else {
    try {
      const pagedStories = await fetchTagStories(tag, currentPage)
      stories = await attachAuthorAvatars(pagedStories.stories)
      hasNextPage = pagedStories.hasNextPage
    } catch {
      feedError = 'Unable to load stories for this tag right now. Please try again shortly.'
    }
  }
  ...
  const pathname = tag ? `/tag/${encodeURIComponent(tag)}` : '/tag'
      ...
      <HomeChrome>
        <section className="rounded-2xl border border-border bg-surface p-4 shadow-card sm:p-6">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3 border-b border-border pb-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-muted">Tag</p>
              <h2 className="text-2xl font-semibold text-text">{tag ?? 'Unknown tag'}</h2>
              <p className="mt-1 text-sm text-muted">Most recent stories carrying this tag.</p>
            </div>
            {tag ? (
              <Link
          ...
          {feedError ? (
            ...
            <p className="text-sm text-muted">No stories found for this tag.</p>
          ) : (
